# typed: true
# frozen_string_literal: true


class FastpixClient::Models::Components::GetAllMediaResponse
  extend ::Crystalline::MetadataFields::ClassMethods
end


class FastpixClient::Models::Components::GetAllMediaResponse
  def id(); end
  def id=(str_); end
  def source_media_id(); end
  def source_media_id=(str_); end
  def workspace_id(); end
  def workspace_id=(str_); end
  def stream_id(); end
  def stream_id=(str_); end
  def media_quality(); end
  def media_quality=(str_); end
  def creator_id(); end
  def creator_id=(str_); end
  def status(); end
  def status=(str_); end
  def mp4_support(); end
  def mp4_support=(str_); end
  def playback_ids(); end
  def playback_ids=(str_); end
  def tracks(); end
  def tracks=(str_); end
  def summary(); end
  def summary=(str_); end
  def chapters(); end
  def chapters=(str_); end
  def named_entities(); end
  def named_entities=(str_); end
  def moderation(); end
  def moderation=(str_); end
  def duration(); end
  def duration=(str_); end
  def frame_rate(); end
  def frame_rate=(str_); end
  def created_at(); end
  def created_at=(str_); end
  def updated_at(); end
  def updated_at=(str_); end
  def thumbnail(); end
  def thumbnail=(str_); end
  def metadata(); end
  def metadata=(str_); end
  def title(); end
  def title=(str_); end
  def max_resolution(); end
  def max_resolution=(str_); end
  def source_resolution(); end
  def source_resolution=(str_); end
  def source_access(); end
  def source_access=(str_); end
  def generated_subtitles(); end
  def generated_subtitles=(str_); end
  def is_audio_only(); end
  def is_audio_only=(str_); end
  def subtitle_available(); end
  def subtitle_available=(str_); end
  def aspect_ratio(); end
  def aspect_ratio=(str_); end
end